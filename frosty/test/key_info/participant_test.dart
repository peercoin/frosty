...
void main() {
  group("ParticipantKeyInfo", () {
    ...
    test("invalid participant info arguments", () {
      ...
      expect(
        () => ParticipantKeyInfo(
          group: GroupKeyInfo(groupKey: groupPublicKey, threshold: 4),
    ...
    test("invalid private key construct", () {
      void expectSharesThrow<Err>(PrivateShareList shares) =>
          expect(() => info.constructPrivateKey(shares), throwsA(isA<Err>()));
      ...
      // Wrong shares
      expectSharesThrow<InvalidShares>([(ids[2], privateShares[1])]);
    });
