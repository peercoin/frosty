import 'package:coinlib/coinlib.dart';
import 'package:frosty/frosty.dart';
import 'package:test/test.dart';

void main() {
  group("DkgCommitmentSet", () {

    late CommitmentPair id1Pair, id2Pair, id3Pair;
    setUp(() async {

      await loadFrosty();

      id1Pair = (
        Identifier.fromUint16(1),
        DkgPublicCommitment.fromBytes(
          hexToBytes(
            "",
          ),
        )
      );

      id2Pair = (
        Identifier.fromUint16(2),
        DkgPublicCommitment.fromBytes(
          hexToBytes(
            "",
          ),
        )
      );

      id3Pair = (
        Identifier.fromUint16(3),
        DkgPublicCommitment.fromBytes(
          hexToBytes(
            "",
          ),
        )
      );

    });

    test("provides expected hash regardless of order", () {
      void expectHash(CommitmentList list) => expect(
        bytesToHex(DkgCommitmentSet(list).hash),
        "4a7605b740be9a37b5b88bc6ce21173997d0e2581f8caa30b760e38537a79610",
      );
      expectHash([id1Pair, id2Pair, id3Pair]);
      expectHash([id3Pair, id2Pair, id1Pair]);
    });

  });
}
