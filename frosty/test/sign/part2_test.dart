...
void main() {
  group("SignPart2", () {
    ...
    test("provides the same signature share per participant each time", () {
      ...
      for (int i = 0; i < 2; i++) {
        final participantShares = List.generate(
          2,
          (_) => cl.bytesToHex(getShare(part1s, i).toBytes()),
        );
    ...
    test("invalid inputs", () {
      void expectInvalid(void Function() f) =>
          expect(() => f(), throwsA(isA<InvalidSignPart2>()));
      ...
      expectInvalid(
        () => getShare(
          part1s,
          0,
          commitmentMap: {ids.first: part1s.first.commitment},
        ),
      );
    });
  });
